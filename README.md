...
To create a Google Cloud Platform (GCP) CodeBuild pipeline to deploy a React.js application on an NGINX server, you'll 
...
Modify the NGINX configuration to serve your React app. Typically, you'll edit the default server block in 
...
In your React.js project repository, create a cloudbuild.yaml file to define the build steps. This file tells Cloud 

Build how to build your React app and deploy it to the NGINX server.
...
After the build completes, visit the IP or domain name of your Compute Engine instance to verify the React app is 

served correctly via NGINX.
...
The error Permission denied (publickey) indicates that the Google Cloud Build service account doesn't have the 

necessary permissions or access to SSH into the Compute Engine instance. This can happen for several reasons, such as 

missing SSH keys, incorrect IAM roles, or issues with OS Login.
...
Check if SSH keys are properly set up: Ensure that the SSH keys are correctly propagated to the VM. This might require 

a few minutes when first set up.
...
3. anually Add SSH Keys to the Instance Metadata

If you're not using OS Login, you might need to manually add the SSH key for the Cloud Build service account to the 
instance's metadata.
...
Run the following command on your local machine (or Cloud Shell) to generate an SSH key pair if you don’t already have 
one:
